/**
 * Mock data layer simulating backend API responses.
 * Replace this with real API calls in services/ when backend is ready.
 */

import type { Company, CompanyAnalysisResponse, KpiRow, ReportSection } from '../types';

// ---------------------------------------------------------------------------
// Companies (indexed by ticker for lookup)
// ---------------------------------------------------------------------------
...
const MOCK_COMPANIES: Record<string, Company> = {
  ...
  // NZ companies – structure allows adding more
  // SKK: { ticker: 'SKK', name: '...', exchange: 'NZX', ... },
  // BPG: { ticker: 'BPG', name: '...', exchange: 'NZX', ... },
};

// ---------------------------------------------------------------------------
// Analysis content per company (company-specific narratives and KPIs)
// ---------------------------------------------------------------------------

function buildKpiRows(rows: Array<{ metric: string; periodValues: Record<string, string>; trend?: KpiRow['trend'] }>): KpiRow[] {
...
/** Get mock analysis by ticker; returns default company's analysis if not found. */
...
/** Full mock response: company + analysis. Used by the data service. */
...
/** List of tickers that have mock data (for type-safe defaults and placeholders). */
